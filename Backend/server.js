...
app.post("/signup", async (req, res) => {
  ...
  pool.query(
    ...
    [name, email, hashed],
    (err) => {
      if (err) return res.status(400).json({ msg: "Email exists or processing error" });
...
app.post("/booking", (req, res) => {
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ msg: "Database insert failed" });
      res.status(201).json({ msg: "Booking stored", bookingId: result.rows[0].id });
    }
  );
});
